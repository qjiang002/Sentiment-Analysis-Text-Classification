...
if args.model == "char_cnn":
    test_x, test_y, alphabet_size = build_char_dataset(DATASET, "test", "char_cnn", CHAR_MAX_LEN)
elif args.model == "vd_cnn":
    test_x, test_y, alphabet_size = build_char_dataset(DATASET, "test", "vdcnn", CHAR_MAX_LEN)
else:
    word_dict = build_word_dict(DATASET, WORD_MAX_LEN)
    test_x, test_y = build_word_dataset(DATASET, "test", word_dict, WORD_MAX_LEN)

text_path = os.path.join(args.model, DATASET)
checkpoint_file = tf.train.latest_checkpoint(text_path)
graph = tf.Graph()
