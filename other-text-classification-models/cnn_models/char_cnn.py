...
class CharCNN(object):
    def __init__(self, alphabet_size, document_max_len, num_class, learning_rate, l2_reg_lambda):
        ...
        self.dropout_keep_prob = tf.where(self.is_training, 0.5, 1.0)
        ...
        with tf.name_scope("fc-1"):
            ...
            fc1_out_drop = tf.nn.dropout(fc1_out, self.dropout_keep_prob)

        with tf.name_scope("fc-2"):
            fc2_out = tf.layers.dense(fc1_out_drop, 1024, activation=tf.nn.relu, kernel_initializer=self.kernel_initializer)
            fc2_out_drop = tf.nn.dropout(fc2_out, self.dropout_keep_prob)
        ...
        with tf.name_scope("fc-3"):
            ...
            self.logits = tf.nn.xw_plus_b(fc2_out_drop, W, b, name="logits")
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits, labels=self.y_one_hot)) + l2_reg_lambda * l2_loss
