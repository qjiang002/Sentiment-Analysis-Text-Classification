...
flags.DEFINE_string(
    "task_name", "classification", "The name of the task to train."
)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      #logging_hook = tf.train.LoggingTensorHook({"training loss": total_loss, "training accuracy": accuracy}, every_n_iter=10)
      ...
      #logging_hook = tf.train.LoggingTensorHook({"eval loss": eval_metrics.eval_loss, "eval accuracy": eval_metrics.eval_accuracy}, every_n_iter=10)
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def main(_):
  ...
  label_list = processor.get_labels(os.path.join(FLAGS.data_dir, FLAGS.train_txt))
